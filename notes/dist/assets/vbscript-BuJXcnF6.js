function p(h) {
  ...
  function l(e, n) {
    ...
    if (e.match(/^((&H)|(&O))?[0-9\.]/i, false) && !e.match(/^((&H)|(&O))?[0-9\.]+[a-z_]/i, false)) {
      var t = false;
      if ((e.match(/^\d*\.\d+/i) || e.match(/^\d+\.\d*/) || e.match(/^\.\d+/)) && (t = true), t) return e.eat(/J/i), "number";
      var o = false;
      if (e.match(/^&H[0-9a-f]+/i) || e.match(/^&O[0-7]+/i) ? o = true : e.match(/^[1-9]\d*F?/) ? (e.eat(/J/i), o = true) : e.match(/^0(?![\dx])/i) && (o = true), o) return e.eat(/L/i), "number";
    }
    return e.match(M) ? (n.tokenize = _(e.current()), n.tokenize(e, n)) : e.match(g) || e.match(y) || e.match(S) ? "operator" : e.match(m) ? null : e.match(w) ? "bracket" : e.match(V) ? (n.doInCurrentLine = true, "keyword") : e.match(q) ? (f(e, n), n.doInCurrentLine = true, "keyword") : e.match(P) ? (n.doInCurrentLine ? n.doInCurrentLine = false : f(e, n), "keyword") : e.match(b) ? "keyword" : e.match(s) ? (u(e, n), u(e, n), "keyword") : e.match(d) ? (n.doInCurrentLine ? n.doInCurrentLine = false : u(e, n), "keyword") : e.match(W) ? "keyword" : e.match(z) ? "atom" : e.match(N) ? "variableName.special" : e.match(A) || e.match(K) ? "builtin" : e.match(k) ? "variable" : (e.next(), v);
  }
  function _(e) {
  ...
  return { name: "vbscript", startState: function() {
    return { tokenize: l, lastToken: null, currentIndent: 0, nextLineIndent: 0, doInCurrentLine: false, ignoreKeyword: false };
  }, token: function(e, n) {
