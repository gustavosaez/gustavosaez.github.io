import { a as G, E as A, s as N, C as I, t as p, o as Y, L as j, g as k, p as U, b as Z, i as B, f as D, u as M } from "./index-CgUHbQAW.js";
...
const ae = new I({ start: null, shift(e, t, o, O) {
...
function ge(e, t) {
  ...
  if (r && (r.to > t || r.lastChild.type.isError)) {
    let a = r.parent;
    if (O.name == "TagName") return r.name == "CloseTag" || r.name == "MismatchedCloseTag" ? { type: "closeTag", from: O.from, context: a } : { type: "openTag", from: O.from, context: C(a) };
...
function pe(e, t) {
  ...
  for (let n = 0; n < a.length; n++) {
    let m = e[n], c = a[n];
    if (m.children) for (let u of m.children) f[u] && c.children.push(f[u]);
    else c.children = a;
  }
  return (n) => {
    var m;
    ...
    if (l == "openTag") {
      ...
      let i = P(c, S);
      return i ? { from: d, to: n.pos + (c.sliceString(n.pos, n.pos + 1) == ">" ? 1 : 0), options: [((m = f[i]) === null || m === void 0 ? void 0 : m.closeNameCompletion) || { label: i + ">", type: "type" }], validFor: b } : null;
    } else if (l == "attrName") {
      let i = f[T(c, S)];
      ...
      let i = Se(c, S, d);
      if (!i) return null;
      let $ = f[T(c, S)], g = (($ == null ? void 0 : $.attrValues) || r)[i];
      return !g || !g.length ? null : { from: d, to: n.pos + (c.sliceString(n.pos, n.pos + 1) == '"' ? 1 : 0), options: g, validFor: /^"[^"]*"?$/ };
    } else if (l == "tag") {
      let i = P(c, S), $ = f[i], g = [], q = S && S.lastChild;
      i && (!q || q.name != "CloseTag" || T(c, q) != i) && g.push($ ? $.closeCompletion : { label: "</" + i + ">", type: "type", boost: 2 });
      let v = g.concat((($ == null ? void 0 : $.children) || (S ? a : s)).map((x) => x.openCompletion));
      if (S && ($ == null ? void 0 : $.text.length)) {
        let x = S.firstChild;
...
const w = j.define({ name: "xml", parser: me.configure({ props: [B.add({ Element(e) {
...
} }), M.add({ "OpenTag CloseTag": (e) => e.getChild("TagName") })] }), languageData: { commentTokens: { block: { open: "<!--", close: "-->" } }, indentOnInput: /^\s*<\/$/ } });
function qe(e = {}) {
  let t = [w.data.of({ autocomplete: pe(e.elements || [], e.attributes || []) })];
  return e.autoCloseTags !== false && t.push(fe), new Z(w, t);
}
...
const fe = Y.inputHandler.of((e, t, o, O, r) => {
  if (e.composing || e.state.readOnly || t != o || O != ">" && O != "/" || !w.isActiveAt(e.state, t, -1)) return false;
  let a = r(), { state: s } = a, f = s.changeByRange((n) => {
    ...
    if (d && O == ">" && S.name == "EndTag") {
      ...
      if (S.from == l - 2 && ((u = $.lastChild) === null || u === void 0 ? void 0 : u.name) != "CloseTag" && (i = z(s.doc, $, l))) {
        let g = l + (s.doc.sliceString(l, l + 1) === ">" ? 1 : 0), q = `${i}>`;
        return { range: U.cursor(l + q.length, -1), changes: { from: l, to: g, insert: q } };
      }
    }
    return { range: n };
  });
  return f.changes.empty ? false : (e.dispatch([a, s.update(f, { userEvent: "input.complete", scrollIntoView: true })]), true);
});
export {
  fe as autoCloseTags,
