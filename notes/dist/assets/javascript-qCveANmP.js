function fr(x) {
  ...
  function S(r, e) {
    ...
    if (t == "/") return r.eat("*") ? (e.tokenize = H, H(r, e)) : r.eat("/") ? (r.skipToEnd(), b("comment", "comment")) : ce(r, e, 1) ? (qr(r), r.match(/^\b(([gimyus])(?![gimyus]*\2))+\b/), b("regexp", "string.special")) : (r.eat("="), b("operator", "operator", r.current()));
    if (t == "`") return e.tokenize = L, L(r, e);
    if (t == "#" && r.peek() == "!") return r.skipToEnd(), b("meta", "meta");
    if (t == "#" && r.eatWhile(U)) return b("variable", "property");
    if (t == "<" && r.match("!--") || t == "-" && r.match("->") && !/\S/.test(r.string.slice(0, r.start))) return r.skipToEnd(), b("comment", "comment");
    if (hr.test(t)) return (t != ">" || !e.lexical || e.lexical.type != ">") && (r.eat("=") ? (t == "!" || t == "=") && r.eat("=") : /[<>*+\-|&?]/.test(t) && (r.eat(t), t == ">" && r.eat(t))), t == "?" && r.eat(".") ? b(".") : b("operator", "operator", r.current());
    if (U.test(t)) {
      r.eatWhile(U);
      var f = r.current();
      if (e.lastType != ".") {
        ...
        if (f == "async" && r.match(/^(\s|\/\*([^*]|\*(?!\/))*?\*\/)*[\[\(\w]/, false)) return b("async", "keyword", f);
      }
  ...
  function ar(r, e) {
    ...
    if (!(t < 0)) {
      ...
      for (var u = 0, c = false, m = t - 1; m >= 0; --m) {
        ...
        else if (U.test(y)) c = true;
        else if (/["'\/`]/.test(y)) for (; ; --m) {
          if (m == 0) return;
  ...
  var Fr = { atom: true, number: true, variable: true, string: true, regexp: true, this: true, import: true, "jsonld-keyword": true };
  function xr(r, e, t, f, u, c) {
    this.indented = r, this.column = e, this.type = t, this.prev = u, this.info = c, f != null && (this.align = f);
  }
  function Jr(r, e) {
    for (var t = r.localVars; t; t = t.next) if (t.name == e) return true;
    for (var f = r.context; f; f = f.prev) for (var t = f.vars; t; t = t.next) if (t.name == e) return true;
  }
  ...
  function w(r, e) {
    return r == "var" ? n(s("vardef", e), mr, l(";"), a) : r == "keyword a" ? n(s("form"), sr, w, a) : r == "keyword b" ? n(s("form"), w, a) : r == "keyword d" ? i.stream.match(/^\s*$/, false) ? n() : n(s("stat"), N, l(";"), a) : r == "debugger" ? n(l(";")) : r == "{" ? n(s("}"), Y, rr, a, T) : r == ";" ? n() : r == "if" ? (i.state.lexical.info == "else" && i.state.cc[i.state.cc.length - 1] == a && i.state.cc.pop()(), n(s("form"), sr, w, a, jr)) : r == "function" ? n(_) : r == "for" ? n(s("form"), Y, zr, w, T, a) : r == "class" || k && e == "interface" ? (i.marked = "keyword", n(s("form", r == "class" ? r : e), Sr, a)) : r == "variable" ? k && e == "declare" ? (i.marked = "keyword", n(w)) : k && (e == "module" || e == "enum" || e == "type") && i.stream.match(/^\s*\w/, false) ? (i.marked = "keyword", e == "enum" ? n($r) : e == "type" ? n(_r, l("operator"), d, l(";")) : n(s("form"), V, l("{"), s("}"), rr, a, a)) : k && e == "namespace" ? (i.marked = "keyword", n(s("form"), p, w, a)) : k && e == "abstract" ? (i.marked = "keyword", n(w)) : n(s("stat"), Kr) : r == "switch" ? n(s("form"), sr, l("{"), s("}", "switch"), Y, rr, a, a, T) : r == "case" ? n(p, l(":")) : r == "default" ? n(l(":")) : r == "catch" ? n(s("form"), E, Lr, w, a, T) : r == "export" ? n(s("stat"), fe, a) : r == "import" ? n(s("stat"), ue, a) : r == "async" ? n(w) : e == "@" ? n(p, w) : o(s("stat"), p, l(";"), a);
  }
  function Lr(r) {
    if (r == "(") return n(O, l(")"));
  }
  ...
  function B(r, e, t) {
    ...
    if (r == "operator") return /\+\+|--/.test(e) || k && e == "!" ? n(f) : k && e == "<" && i.stream.match(/^([^<>]|<[^<>]*>)*>\s*\(/, false) ? n(s(">"), h(d, ">"), a, f) : e == "?" ? n(p, l(":"), u) : n(u);
  ...
  function Vr(r) {
    if (k && r == ":") return i.stream.match(/^\s*\w+\s+is\b/, false) ? n(p, Xr, d) : n(d);
  }
  function Xr(r, e) {
  ...
  function j(r, e) {
    if (r == "async" || r == "variable" && (e == "static" || e == "get" || e == "set" || k && X(e)) && i.stream.match(/^\s+#?[\w$\xa1-\uffff]/, false)) return i.marked = "keyword", n(j);
  ...
  return { name: x.name, startState: function(r) {
    var e = { tokenize: S, lastType: "sof", cc: [], lexical: new xr(-r, 0, "block", false), localVars: x.localVars, context: x.localVars && new P(null, null, false), indented: 0 };
    ...
    for (; (u.type == "stat" || u.type == "form") && (f == "}" || (c = r.cc[r.cc.length - 1]) && (c == I || c == B) && !/^[,\.=+\-*:?[\(]/.test(e)); ) u = u.prev;
  ...
  }, languageData: { indentOnInput: /^\s*(?:case .*?:|default:|\{|\})$/, commentTokens: br ? void 0 : { line: "//", block: { open: "/*", close: "*/" } }, closeBrackets: { brackets: ["(", "[", "{", "'", '"', "`"] }, wordChars: "$" } };
}
